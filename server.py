...
def toponym_by_geocode(geocode):
    try:
        yandex_req = requests.get(
            f'http://geocode-maps.yandex.ru/1.x/?apikey=40d1649f-0493-4b70-98ba-98533de7710b&geocode={geocode}&lang'
            f'=en_US'
            f'&format=json')
...
def url_image_by_ll(ll):
    yandex_req = requests.get(
        f'http://static-maps.yandex.ru/1.x/?ll={ll}&spn=10,1&lang=en_US&size=200,200&l=map&pt={ll},flag')
...
def news_main():
    ...
    if geoform.validate_on_submit():
        ...
        if toponym == 'error':
            geoform.geoinfo = {'map': 'error', 'address': 'error', 'image': 'error'}
        else:
